...
function BodySizeSet({ route, navigation }) {
    ...
    useEffect(() => {
        axios.get(API_URL + '/list/variables', { headers: { token } }).then((resp) => {
                ...
                if (resp.data.status === 'success') {
                    ...
                    EncryptedStorage.setItem('user_meta', JSON.stringify({ 'bodyStatus': true })).then(() => {
                        Alert.alert('Bilgi', 'Beden ölçüleriniz başarı ile kaydedildi!', [
                            { text: "Tamam", onPress: () => navigation.navigate('Root'), }
                        ]);
                    }).catch((error) => {
                        Alert.alert('Uyarı', 'Bilinmeyen bir sorun oluştu. Lütfen tekrar deneyin!', [
                            { text: "Tamam", onPress: () => { }, }
        ...
        <React.Fragment>
            {variables._wait ? (
                ...
                <KeyboardAwareScrollView style={{ backgroundColor: colors.background }}>
                    <SignLayout>
                        <Text style={styles.appName}>FULLBODYGYM</Text>
                        <Text style={styles.descriptionText}>Uygulamayı kullanabilmek için beden ölçülerinizi girin</Text>
                    </SignLayout>
                    <View style={{ paddingLeft: 15, paddingRight: 15, marginBottom: 30 }}>
                        ...
                        <Button mode="contained" onPress={form._wait ? null : handleSave} style={{ backgroundColor: colors.secondary, marginTop: 15 }}>
